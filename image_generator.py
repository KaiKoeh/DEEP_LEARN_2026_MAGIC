...
PERSPECTIVE_SHIFT = 0.15             # KIPP WINKEL DER PERSPEKTIVE
PERSPECTIVE_PAD = 0.2                # extra Rand zum Wegschneiden, da Schwarze Ränder beim Kippen

######## COLOR EFFEKT EBENE

## ÜBER DAS BILD
NOISE_RANDOM = (0.02, 0.8)          # NOISE EFFEKT RANDOM INTENSIVITÄT
MOTION_BLUR = (3, 10)               # VERWACKELUNGS EFFEKT RANDOM INTENSIVITÄT
OVERALL_BRIGHTNESS = (0.6, 1.4)     # HELLIGKEIT > ÜBER DAS GESAMTE BILD
OVERALL_CONTRAST = (0.7, 1.3)       # KONTRAST > ÜBER DAS GESAMTE BILD
OVERALL_SATURATION = (0.5, 1.5)     # SÄTTIGUNG > ÜBER DAS GESAMTE BILD
...
for bg in backgrounds:
    ...
    if w < BG_CANVAS_SIZE or h < BG_CANVAS_SIZE:
        scale = BG_CANVAS_SIZE / min(w, h)
        img = Image.fromarray(bg).resize((int(w * scale), int(h * scale)), Image.LANCZOS)
        bg = np.array(img)
        h, w = bg.shape[:2]

    cx = (w - BG_CANVAS_SIZE) // 2
    cy = (h - BG_CANVAS_SIZE) // 2
    canvas = bg[cy:cy + BG_CANVAS_SIZE, cx:cx + BG_CANVAS_SIZE].copy()
    ...
    for v in range(BACKGROUND_VARIATIONS):
        ...
        left = max(0, min(cx + shift_x, w - BG_CANVAS_SIZE))
        top = max(0, min(cy + shift_y, h - BG_CANVAS_SIZE))


        crop = bg[top:top + BG_CANVAS_SIZE, left:left + BG_CANVAS_SIZE].copy()
        ...
        zoomed_size = int(BG_CANVAS_SIZE * zoom)
        crop_img = crop_img.resize((zoomed_size, zoomed_size), Image.LANCZOS)

        zl = (zoomed_size - BG_CANVAS_SIZE) // 2 + random.randint(-30, 30)
        zt = (zoomed_size - BG_CANVAS_SIZE) // 2 + random.randint(-30, 30)
        zl = max(0, min(zl, zoomed_size - BG_CANVAS_SIZE))
        zt = max(0, min(zt, zoomed_size - BG_CANVAS_SIZE))

        crop_img = crop_img.crop((zl, zt, zl + BG_CANVAS_SIZE, zt + BG_CANVAS_SIZE))
...
for canvas in bg_canvases:
    for _ in range(CARDS_PER_CANVAS):
        ...
        card_h = int(BG_CANVAS_SIZE * scale)
        ...
        pad = int(BG_CANVAS_SIZE * CARD_PADDING)
        max_x = BG_CANVAS_SIZE - rot_w - pad
        max_y = BG_CANVAS_SIZE - rot_h - pad
        ...
        xc = (pos_x + rot_w / 2) / BG_CANVAS_SIZE
        yc = (pos_y + rot_h / 2) / BG_CANVAS_SIZE
        bw = rot_w / BG_CANVAS_SIZE
        bh = rot_h / BG_CANVAS_SIZE
...
for i in range(len(generated_images)):
    ...
    pad_px = int(BG_CANVAS_SIZE * PERSPECTIVE_PAD)
    big_size = BG_CANVAS_SIZE + 2 * pad_px
    big_img = np.array(
    ...
    generated_images[i] = warped[pad_px:pad_px + BG_CANVAS_SIZE, pad_px:pad_px + BG_CANVAS_SIZE]
    ...
    new_x2 = min(BG_CANVAS_SIZE, transformed[:, 0].max() - pad_px)
    new_y2 = min(BG_CANVAS_SIZE, transformed[:, 1].max() - pad_px)

    generated_labels_bbox[i] = [
        (new_x1 + new_x2) / 2 / BG_CANVAS_SIZE,
        (new_y1 + new_y2) / 2 / BG_CANVAS_SIZE,
        (new_x2 - new_x1) / BG_CANVAS_SIZE,
        (new_y2 - new_y1) / BG_CANVAS_SIZE
    ]
...
print(f"{len(backgrounds)} Hintergründe auf {BG_CANVAS_SIZE}x{BG_CANVAS_SIZE} zugeschnitten")
...
for i in range(min(12, len(generated_images))):
    ...
    # BBox zeichnen
    xc, yc, bw, bh = generated_labels_bbox[i]
    x = (xc - bw / 2) * BG_CANVAS_SIZE
    y = (yc - bh / 2) * BG_CANVAS_SIZE
    ax.add_patch(patches.Rectangle(
        (x, y), bw * BG_CANVAS_SIZE, bh * BG_CANVAS_SIZE,
